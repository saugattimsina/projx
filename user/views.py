...
def change_user_password(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "users/password_change_user.html",
                context={"form": form, "id": user_obj.id},
            )
    else:
        ...
        if form.is_valid():
            ...
            if user and user.is_staff:
                ...
                return render(
                    request,
                    self.template_name,
                    {"form": form, "message": "username or password is incorrect"},
                )
        else:
            # Handle the case when the form is not valid
            # You might want to render the form again with errors
            return render(
                request, self.template_name, {"form": form, "message": "invalid data"}
            )
